function fallen_moon_damage(event)
	...
	if is_this_a_crit == true then
		...
		--GameRules:SendCustomMessage("[ Debug ] fallen_moon_damage(critical): " .. damage_after_crit, 0, 0)
		...
		ApplyDamage(damageTable)
		--GameRules:SendCustomMessage("[ Debug ] fallen_moon_damage: " .. damage_value, 0, 0)
	end
end



function fallen_moon_damage_aoe(event)
	local caster = event.caster
	local ability = event.ability
	local target = event.target

	local current_int = caster:GetIntellect()
	local int_multiplier = ability:GetLevelSpecialValueFor("pct_int_to_dmg_aoe", ability:GetLevel())
	local int_multiplier_float = int_multiplier/100
	--GameRules:SendCustomMessage("[ Debug ] ability:GetLevel(): " .. ability:GetLevel(), 0, 0)
	--GameRules:SendCustomMessage("[ Debug ] int_multiplier_float: " .. int_multiplier_float, 0, 0)
