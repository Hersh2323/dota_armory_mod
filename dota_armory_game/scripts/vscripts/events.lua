...
function barebones:OnNPCSpawned(keys)
	...
	if npc:IsRealHero() and npc.bFirstSpawned == nil then
	    ...
	    c_func_respawn_npcs()
	    local spawner_a = Entities:FindByName(nil, "npc_armory_basic_creep_badguys_ranged_datadriven")
...
function barebones:OnHeroInGame(hero)
    ...
    if hero:GetPrimaryAttribute() == DOTA_ATTRIBUTE_INTELLECT then
        local item = CreateItem("item_i3_main_stat_int_staff_datadriven", hero, hero)
        hero:AddItem(item)
    else
    end

    if hero:GetPrimaryAttribute() == DOTA_ATTRIBUTE_AGILITY then
        local item = CreateItem("item_i3_main_stat_agi_dagger_datadriven", hero, hero)
        hero:AddItem(item)
    else
    end

    if hero:GetPrimaryAttribute() == DOTA_ATTRIBUTE_STRENGTH then
        local item = CreateItem("item_i3_main_stat_str_sword_datadriven", hero, hero)
        hero:AddItem(item)
    else
    end

    if IsInToolsMode() then
        local item = CreateItem("item_i6_main_stat_str_sword_datadriven", hero, hero)
        hero:AddItem(item)
        local item = CreateItem("item_i6_boots_crystal_datadriven", hero, hero)
        hero:AddItem(item)
        local item = CreateItem("item_i6_crystal_datadriven", hero, hero)
        hero:AddItem(item)
        local item = CreateItem("item_i6_crystal_datadriven", hero, hero)
        hero:AddItem(item)
        local item = CreateItem("item_i6_crystal_datadriven", hero, hero)
        hero:AddItem(item)
        local item = CreateItem("item_i6_berserking_bracers_datadriven", hero, hero)
        hero:AddItem(item)
    ...
    local ench_1 = hero:FindAbilityByName("enchantress_eye_of_the_forest_datadriven")
    if ench_1 then ench_1:SetLevel(ench_1:GetLevel()+1) else end
    local ench_2 = hero:FindAbilityByName("enchantress_rejuvinate_datadriven")
    if ench_2 then ench_2:SetLevel(ench_2:GetLevel()+1) else end
    ...
    local abil_talents = hero:GetAbilityByIndex(17)
    --print("[debug][abil_talents]" .. abil_talents:GetName())
    hero:RemoveAbility("special_bonus_attributes")
end
...
function barebones:OnItemPickedUp(keys)
	...
	if unit_entity:GetPrimaryAttribute() == DOTA_ATTRIBUTE_INTELLECT then

	    if string.find(itemNameString, "main_stat_str_sword") then
	        --print("unit has picked up an item with string.find _chest_ in name")
	        local pos = itemEntity:GetAbsOrigin()
	        local pos_launch = pos+RandomVector(RandomFloat(125,175))
	        unit_entity:DropItemAtPositionImmediate(itemEntity, pos_launch)
	    else
	    end

	    if string.find(itemNameString, "main_stat_agi_dagger") then
	        --print("unit has picked up an item with string.find _chest_ in name")
	        local pos = itemEntity:GetAbsOrigin()
	        local pos_launch = pos+RandomVector(RandomFloat(125,175))
	        unit_entity:DropItemAtPositionImmediate(itemEntity, pos_launch)
	    ...
	    if string.find(itemNameString, "main_stat_int_staff") then
	        --print("unit has picked up an item with string.find _chest_ in name")
	        for itemSlot=0,17 do
	            local items = unit_entity:GetItemInSlot(itemSlot)
	            if items then
	                --print(items:GetName())
	                if string.find(items:GetName(), "main_stat_int_staff") then

	                    if items == itemEntity then
	                    else
	                        local pos = itemEntity:GetAbsOrigin()
	                        local pos_launch = pos+RandomVector(RandomFloat(125,175))
	                        unit_entity:DropItemAtPositionImmediate(items, pos_launch)
	                        --print("string.find passed _chest_")
	                        --print("items:GetName()" .. items:GetName())
	                    end
	                end
	            end
	        end
	...
	if unit_entity:GetPrimaryAttribute() == DOTA_ATTRIBUTE_AGILITY then
	    ...
	    if string.find(itemNameString, "main_stat_agi_dagger") then
	        ...
	        for itemSlot=0,17 do
	            ...
	            if items then
	                --print(items:GetName())
	                if string.find(items:GetName(), "main_stat_agi_dagger") then
	...
	if unit_entity:GetPrimaryAttribute() == DOTA_ATTRIBUTE_STRENGTH then
	    ...
	    if string.find(itemNameString, "main_stat_str_sword") then
	        ...
	        for itemSlot=0,17 do
	            ...
	            if items then
	                --print(items:GetName())
	                if string.find(items:GetName(), "main_stat_str_sword") then
...
function barebones:OnPlayerLevelUp(keys)
    ...
    if GLOBAL_VARIABLE_CURRENT_HERO_LEVEL then
        if level > GLOBAL_VARIABLE_CURRENT_HERO_LEVEL then
            ...
            print("[ DEBUG ] GLOBAL_VARIABLE_CURRENT_HERO_LEVEL has increased to: " .. GLOBAL_VARIABLE_CURRENT_HERO_LEVEL)
            GameRules:SendCustomMessage("[ Game ] Current hero max level increased to: " .. GLOBAL_VARIABLE_CURRENT_HERO_LEVEL .. "", 0, 0)
...
function barebones:OnPlayerPickHero(keys)
    ...
    print("[Debug][Teams Debug][playerid: " .. hero_entity:GetPlayerID() )
    print("[Debug][Teams Debug][team: " .. GetTeamName(hero_entity:GetTeamNumber() ) )
    hero_entity:SetTeam(DOTA_TEAM_GOODGUYS)

    c_func_increment_total_players()
    c_func_start_rescaling_npc_health()
...
function barebones:OnConnectFull(keys)
    ...
    if GameRules:GetGameModeEntity() then
        ...
        print("[debug][GameRules:GetGameModeEntity() exists]")
    else
        print("[debug][GameRules:GetGameModeEntity() does not exist]")

    end

    GameRules:GetGameModeEntity():SetCustomAttributeDerivedStatValue(DOTA_ATTRIBUTE_STRENGTH_DAMAGE, 2.20)
